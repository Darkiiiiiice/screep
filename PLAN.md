...
> 版本:v2.1 · 2026-09-16 · 状态:M0、M1、M2、M3 已完成核心验收,进入 M4(侦察与远矿);M3 的 RCL4 到达与基准入侵实测为线上观察项,M1 的跨阶段压力场景仍由后续里程碑覆盖。
...
| **M3 建造、维护与基础防御 ✅** | 自动布局、分批施工、维修、动态升级、RCL 解锁、tower、防守兵、避险 | M2 | **已完成核心验收**:本地 17 场景变体 133/133 检查全绿(含 progression RCL2→RCL4 分段验收、maintenance 容器维修、defense 塔集火/治疗/盟友豁免);线上 `shard3/W35S2` 实证 extension 0→10/10 自主铺满(容量 300→800)、tower 自主放置并落成充能、双源容器维修闭环、Memory 幽灵清扫(257→6)、降级余量全程未破红线、9 窗口 500+ 采样零错误零回滚,详见 [线上验证](docs/M3-live-validation.md)。RCL4 到达(纯时间函数)与基准入侵实测为观察项,不阻止核心完成 |
...
- [x] M3：防御加固。`runDefense` 接收校验后的 `policy.allies`，`FIND_HOSTILE_CREEPS` 过滤盟友——塔永不对盟友开火（PLAN §2.6）；`TOWER_DEFENSE_RESERVE=100` 让治疗/维修保留开火储备而 attack 不受限；logistics sinks 纳入 tower（优先级 spawn 10 > tower 7 > extension 5），hauler 自动补塔——真实对局中塔不再永久空转。defense 单测 +2（盟友豁免、储备门控）。17 场景变体 128/128 检查保持全绿。
- [x] M3:RCL2→RCL4 连续发展验收。`--progression-probe`（5500 tick）分三段注入 RCL 提升并逐段断言:RCL2 扩展达上限 5、RCL3 塔放置施工、RCL4 storage 放置施工,全程心跳完整零错误;planner 按 ext(≤5)→tower→storage 分批推进（placement 按 owned 计数,塔工地不阻塞 storage),builders 改从物流网络取能修复自采饥饿;人口目标改为慢信号（sources×2 + capacity/300,≤12）不再随 store 振荡。报告:`artifacts/scenarios/fresh-1789440059920-96271/report.json`;18 场景变体 133/133 检查。
...
- [x] M4：SCOUT 纵切（§3.9 远矿流第一步）。`src/domain/intel.ts` 纯决策情报层（observe/isStale/nextScoutTarget/shouldSpawnScout/不可达黑名单 TTL 5000/INTEL_CAP 16 逐旧）；`src/game/intel.ts` 接线：邻房出口候选、scout 巡游观测（sources/controller/threat/mineral）、死亡对账（失踪带目标→不可达）、退役冷却。经济保护三门（回归取证驱动）：RCL≥2（boot 能量不养奢侈品）+ 孵化满能 300 + 死亡冷却 100——裸 [MOVE] scout 曾因 50×4 连续幻影死亡抽血 200 能量把 `test:logistics` 600 tick 升级进度 116→50 打出阈值，二分（stash 对比）确证后加门。引擎探针 `--intel-probe`：3×3 种子（W0N1 home + W0N2 中性双源）断言 scout 出生/进邻房/双源记录/无伪造归属；`Game.map.describeExits` 对未种子幻影房间抛异常由 try/catch 兜底转黑名单。16 场景变体全绿（容器存量检查同步再校准为时间窗判定，见台账）。
- [x] M4：专职矿工纵切（§3.2 经济效率）。`minerSpawnNeed` 纯决策四门：容量≥650 + 能量全额 650 + 工人地板 4 + 工人补员让位（与 scout 三门同源，矿工是满员工人口粮外的盈余支出）；身体 [WORK×5, CARRY, MOVE]（引擎实证：无 CARRY 时 harvest 溢出自动丢地上成孤儿堆，必须带一格货架过货，`.engine` harvest.js:50-57）；`runMiners` 驱动：站容器→过货→满采，容器/源丢失回收为通用工不浪费尸体；物流兼任认领对专职源让位（容器只有一个站位，名单经 context 传入不扰 stub）。探针 `--miners-probe`（RCL3+8 满能 ext+双容器+6 工人种子）：t3 孵化、t69 站位、t70 开采全绿。排雷：progression 三度校准（RCL2 段改 [2800,3899] 窗口判定——mock 引擎 CPU 测时混沌，同代码两跑出生差 1 tick 放大为完工时点 ±250 漂移，台账三例统一"点读→窗口证据"）。17 场景变体 + 83 单测全绿。
- [x] M4：远矿评分纵切（§3.9 EVALUATE）。`evaluateRemoteTargets` 纯决策：过期/威胁/归属/预留四过滤 + `sources×100 - distance×10` 评分（字典序决胜，EVAL_TOP=3）；game 层 findRoute 跳数缓存（路由静态不失效）+ `Memory.intel.evaluation` 快照（25 tick 节奏）；本切片只产决策记录，不孵远矿单位。intel 探针扩 2 断言（W0N2 双源当选、贫瘠房不入榜）。**线上首评即实证过滤器价值**：邻房 W35S3（双源"奖品"）实为 nullCorp 所有（RCL5+塔+7 敌兵）、W36S2 为 katarsin 所有（RCL3），双双正确出局；唯一中立房 W35S1（单源无威胁）当选——若盲选双源房即是送死目标。
- [x] M4：预定者纵切（§3.9 CLAIM/RESERVE）。`claimerSpawnNeed` 五门（容量/全额 650/工人地板 4/无在飞/死亡冷却 500）锁定评估榜首，我方预定低于 2000 余量才补刷；`driveClaimers`：scout 同式跨房导航（range 22 + 寻路抛错转黑名单）、目标失效（威胁/被他人预定/情报过期）即退役、**在目标房顺手重观测**——不刷新则情报过期令已预定房间掉出评估、预定断档。探针 `--claim-probe`（RCL3 满配+情报预种子）：预定者赴 W0N2 下预定，`reserver=M0` 经重观测回写。排雷：跨房 moveTo 不带 range 在 mock 寻路层原地卡死（scout 早踩过，同式修复）；快照只含母房对象，跨房证据一律走 memory。18 场景变体 + 86 单测全绿。
- [x] M4：远矿工人纵切（§3.9 DEPLOY，远矿流收官）。`pioneerSpawnNeed` 六门：容量/全额 400/工人地板 4/无在飞/死亡冷却 300/**须我方已预定**（CLAIM 先行，流程不许跳步）；`drivePioneers`：[WORK×2,CARRY×2,MOVE×2] 采满自运回母房喂孵化/容器体系，**威胁撤离保命不自杀、情报复位自动复工**（M4 验收原文），送达量 `pioneerDelivered` 累计供净收益核算。**探针抓出真 bug**：自我预定被评估"预留出局"过滤，CLAIM 一生效流水线自我截断——改为仅他人预留出局（评估签名加 `me`）。排雷两枚：快照点读第三次掷硬币（预定者离房快，出生证据改 memory）；夹具须封死施工排水口（塔工地 5000 进度把能量钉死低位，与线上扩展期同现象）。19 场景变体 + 88 单测全绿。
