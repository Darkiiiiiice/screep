...
> 当前开发计划见 [`../PLAN.md`](../PLAN.md)。本文件保留历史原文，含过期状态、已删除代码引用和待复核结论，不作为当前实现依据。
...
且 `@screeps/driver` 直接 `require('isolated-vm')`，无法绕开。

> **更正（2026-09-11，实测推翻上述结论）**：根因不是 Node 版本，而是**编译器**。
> 组合 **Node 24 + GCC 15** 编译并运行成功，本仓库已在此组合下跑通 2000-tick 真引擎模拟
> （**238 ms/tick，对比线上约 4000 ms/tick，约 17 倍加速**），真实寻路/疲劳/能量流全部可用。
> 复现步骤保留在 `scripts/engine-setup.sh`（`npm run engine`）。
>
> 两次失败都发生在 GCC 16 上，而当时归因于 Node —— 环境有多个编译器时，**先换编译器再换运行时**。
> 放弃本地引擎的代价远大于当初的估计：v1 期间踩到的真 bug（寻路被静止 creep 堵死、
> 缓存路径穿过被占格、spawn 被自家工地围死、3 tick/格身体）**无一能被单测发现**，全部只在引擎物理层可见。
> 重设计不应基于"没有本地引擎"这一前提。
...
> **前提已更正（见 §1.4）**：本地引擎可用，但线上观测仍然是**唯一能回答"真实世界怎么样了"**的信道 ——
> 引擎能验证物理，房间里其他玩家、矿物分布、邻居威胁、真实网络延迟不在本地。

线上观测受 §1.2 配额约束，因此观测设计必须在 M1 就位，且**默认省钱**：
...
> **前提更正**：见 §1.4 —— 本地真实引擎**可用**（Node 24 + GCC 15）。本节四层是 v1 在
> 误以为无引擎的前提下设计的；重设计时应把「本地引擎跑真实物理」作为一等验证手段，
> 而不是把它排除在外。

验证由这四层构成。**A 是主力**，C/D 是真实世界校验。
...
| **M3 `BOOTSTRAP`→`ESTABLISHED`** 🔄 | RCL 1–5 | 容器/存储、RCL 升级、builder/upgrader 配比、body 按能量自适应、状态迁移判定 | **进行中**：RCL **1→2 已完成**；**7 结构全部建成**（5 extension + 2 容器，紧邻 source），**hauler 在场**（ESTABLISHED 状态机触发，实测快照 gt=82897645）。RCL 2→3 进度 13,452/45,000（约 30%）。
**容器衰减与维修通道已上线**：容器 10 hits/tick 衰减（引擎 `tick.js` 实测源码），满损 ~25,000 tick ≈ 28h —— 容器归零即退回 BOOTSTRAP，故维修是承重结构。repair 纵切（TaskKind/Intent/REACH/execute/planner/decide）已本地引擎端到端验证（种入 45k-hits 受损容器 → 修至 57.7k，净 +12.7k）。分级：`hits < 50%` 产维修任务；`< 25%`（CRITICAL）时 builder **中断建造**优先维修。-builder 需求条件扩为「有工地或有受损结构」。
⚠️ 「2000 tick 无断档」：本地引擎 2000-tick 12 项全 PASS（deaths=6 → 最长无 harvester 连续段 **0 tick**，SPAWN_RESERVE 生效）；线上需在新代码下重新计量。RCL 3 一到 tower 会自动开建 —— **tower 在 sinksFor 中视为待命弹药**（满仓即停），攻击逻辑属 M5。 |
...
RCL 2 之后连续踩到四个「经济停滞」，根因各不相同，逐个记录以免重犯：

| 现象 | 根因 | 教训 |
|---|---|---|
| 满仓 spawn 旁饿死 | `energySupply` 无条件排除 spawn（想留给补员），但 RCL 1-2 无容器 | **排不空的缓冲区等于没有缓冲区** |
| 死亡螺旋（无法自愈） | 1 harvester 供 4 消费者 → spawn 近零 → harvester 寿命到期后无钱重建收入 → 剩余 creep 抽走每一单位 | 保留 spawn 必须**条件化**：无收入时保留，有收入时放行 |
| 载满能量却无法交付 | 4 个 extension 工地占了 spawn 八邻格中的 4 格（+1 静止 defender），**工地同样阻挡寻路** | 支撑性结构不可"尽可能贴近"枢纽；永久保留其邻圈 |
| creep 两格间往复 40+ tick | `reusePath` 缓存穿过后来被占的格子 | 卡死检测须按**邻域+时长**，不能只比「与上 tick 同格」 |

**当前收入结构（实测）**：harvester 1 个，收入约 2 e/tick；消费者 4 个（2 upgrader + builder + 1 静止 defender），建造单独消耗约 4 e/tick。**消费者多于收入是这些问题的共同土壤** —— 5 个 extension 完工后容量从 300 升至 550，但吞吐仍受限于单个 harvester 的 2 e/tick。

**下一阶段的真实瓶颈已明确：不是容量，是 harvester 数量与移动效率。** RCL 3 的容器（plan §5）正是解药：容器让 harvester 定点采矿、由 hauler 转运，从而把「每人每次往返」压缩为「一次往返多趟」。

**流程防线**：`deploy.mjs` 现在拒绝上传比源码旧的产物 —— 本轮实际踩到「先 deploy 后 build」，上传了旧包，修复「看起来没生效」并浪费一次配额。

### 5.3 两条结构性规则（由实测确立，跨越具体里程碑）

**① 唯一的恢复手段必须有主动保留。** 只有 harvester 会向 spawn 存能量，其余角色都是净消费者 —— 所以 spawn 的能量是殖民地恢复收入的**唯一**手段。两种错误都被实测证明是有害的：无条件排除 spawn 供能 → 消费者在满仓旁饿死 78 tick；事后触发式保留（harvester 全死后才保留）→ 太晚（实测 spawn=14、唯一 harvester 距寿命上限仅 112 tick）。
**定为常量下限 `SPAWN_RESERVE = 250`**（最便宜可用 harvester 身体造价）。它是下限而非锁：满仓 spawn 仍对外开发，否则就退回第一种错误。

**② 结构的锚点由它的用途决定，不能统一。** 实测踩到：选址只以 spawn 为锚，于是容器建在了 spawn 旁（距最近 source 4–6 格），**等于白建** —— 容器的意义是在采矿点缓冲，建在 spawn 旁 harvester 照样走完全程。而 extension 恰好相反：它是投递目标，距 spawn 的距离会重复整个房间生命周期。
故 anchor 按类型分派：`container` → 每个 source 一个锚点；`extension`/`tower`/`storage` → spawn。

**③ 完整生命周期必须在本地引擎里跑过一次。** 2000-tick sim（≈8 分钟，约 17 倍加速）覆盖「creep 到寿命 1500 → 死亡 → 保留线 → 重生」全程，是 600-tick 门禁**结构性做不到**的 —— 死亡螺旋只在 tick 1500 后发生。断言写法有两轮教训：「population > 0」太弱（剩 1 只闲逛 upgrader 也 PASS，而吞吐已塌）；「每 tick 都有 harvester」太严（替身孵化有 12+ tick 合法空窗）。正确形式是**界空窗**：「最长无 harvester 连续段 ≤ 60 tick」，且失败时 dump 窗口明细（起止 tick、在场角色、spawn 能量曲线）作证据。

**④ 占满配额的东西必须能被回收，否则修复会自锁。** `structureWants` 把在建工地计入上限（避免重复排期，正确），但配上「上限恰好被占满」就形成自锁：实测 RCL 2 容器上限 2、线上已有 2 个错位容器工地 → 新逻辑一个也放不下，**部署成功但收益为零**，看起来像修复无效。更糟的是它不会自然解除（那 2 个工地不阻挡连通性，`clearBlockingSites` 不管）。
故凡「把某物计入上限」的规则，都要配一条与之对应的**回收规则**：容器必须紧邻 source（>2 格即回收工地）。接线位置也关键 —— 必须在「无建造需求就提前返回」之前，因为**上限占满时 `wants` 恰好为空**，那正是需要回收的场景。

**④ 未知角色的 creep 不得缺省为任何生产角色。** `viewCreep` 曾用 `memory.role ?? 'harvester'`：无 memory 的 creep（外部 creep、memory 被清的）被算成 harvester 产能。2000-tick 本地引擎实测抓到 —— sim 的阻挡块被算成 harvester，恰好抵消 BOOTSTRAP 的 harvester 需求 1，**殖民地开局 450 tick 无收入**，全靠 upgrader 自采自存（1 e/tick）苟活；600-tick 门禁结构性看不到（空窗恰在 600 内结束），线上也看不到（早已过开局）。规则：`role: string | null`，null = 不可指挥也不计任何配额。

**⑤ 选址用不变量检查，不用几何规则。** 「保留 spawn 的 N 邻格」不足 —— 移动是 8 方向，半径 2 处封特定格子同样能堵死走廊而邻格全空（实测：满载 harvester 往复 40+ tick 无法交付）。改为放置前做 BFS 验证 **spawn 仍能到达 source**，不通则跳过；连通性已破坏时主动移除工地直至恢复。
配套细节：修复步骤必须在「无建造需求」的提前返回**之前**，否则工地已满时永不执行；且**施工工地同样阻挡寻路**。
...
| 风险 | 影响 | 对策 |
|---|---|---|
| ~~无本地真实引擎验证~~ **（已推翻，见 §1.4）** | 物理语义错误曾以为只能在线上暴露 | 本地引擎 Node 24 + GCC 15 可用，238 ms/tick，真物理逐 tick 可测 |
